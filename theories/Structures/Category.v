...
Section Section_Category_Type.
  ...
  Program Definition to_Category : Category_Type → Category :=
    fun ctype =>
      {|
  ...
  Program Definition to_Category_Type : Category → Category_Type :=
    fun C =>
      {|
        cat_object := object C;
...
Section Section_Categories.
  ...
  Lemma Setoids_comp {a b c : Setoids} {f : a ⟶ b} {g : b ⟶ c} : forall x, (g ∘ f) x == g (f x).
  Proof.
    intro.
    reflexivity.
  Qed.
  ...
  Lemma Types_comp {a b c : Types} {f : a ⟶ b} {g : b ⟶ c} : forall x, (g ∘ f) x = g (f x).
  Proof.
    intro.
    reflexivity.
  Qed.
